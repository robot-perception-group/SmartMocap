...
with trange(451,ds.data_lengths[seq_no]-2000,2*(seq_len-overlap)) as seq_t:
    for seq_start in seq_t:
        ...
        smpl_motion_latent.append(torch.zeros(1024).float())

j2ds = torch.cat(j2ds,dim=0)
smpl_motion_latent = torch.stack(smpl_motion_latent).detach().requires_grad_(True)
...
with trange(n_optim_iters) as t:
    for i in t:
        ...
        for j in range(1,smpl_motion.shape[0]):
            pos, ori = geometry.get_ground_point(smpl_motion[j-1,-5,:3],p3d_rt.axis_angle_to_matrix(smpl_motion[j-1,-5,3:6]))
