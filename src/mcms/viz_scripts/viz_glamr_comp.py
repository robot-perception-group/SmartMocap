...
data_glamr = np.load("/is/ps3/nsaini/projects/mcms/mcms_logs/fittings/all_res_wth_sigma/rich_upto_cam1/0000/gt_and_glamr_res_strt_off_10.npz")
...
if len(data["verts"].shape) == 3:
    ...
    smpl_out = np.concatenate([data["verts"],data_glamr["verts"][1:]])
    try:
        cam_trans = np.concatenate([data["cam_trans"],data_glamr["cam_trans"][1:]])
        cam_rots = np.concatenate([data["cam_rots"],data_glamr["cam_rots"]])
        num_cams = cam_trans.shape[1]
        cams_available = True
    except:
        cams_available = False
...
for dat_idx in range(smpl_out.shape[0]):
    ...
    for en_idx,idx in enumerate(motion_range):
        ...
        offsets = np.array([0.5*en_idx,[-3,0,-6][dat_idx],0])
