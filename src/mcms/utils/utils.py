...
def smpl2mop(poses,bm):
    ...
    mop_transfs = torch.zeros(poses.shape[0],22,9).float().to(poses.device)
    mop_transfs[:,:,:6] = p3dt.matrix_to_rotation_6d(transfs[:,:,:3,:3])
    mop_transfs[:,:,6:] = transfs[:,:,:3,3]

    return mop_transfs

def mop2smpl(mop_transfs,bm):
