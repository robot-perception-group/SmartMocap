...
seq_len = big_seq_end - big_seq_start
...
if dset.lower() == "h36m":
    # get batch
    batch = ds.__getitem__(seq_no,seq_start,seq_len=seq_len)
else:
    batch = ds.__getitem__(seq_start,seq_len=seq_len)
...
with torch.no_grad():
    ...
    if config["use_pare_init"]:
        ...
        smpl_art_motion_vp_latent_init = vp_model.encode(batch["pare_poses"][:, :21].reshape(seq_len, 63).to(device)).mean.detach()
        # first frame init
        ...
        smpl_orient_rf = p3d_rt.matrix_to_rotation_6d(smpl_orient[1:2].repeat(seq_len-1,1,1)).detach().requires_grad_(True)
...
with trange(n_optim_iters) as t:
    for i in t:
        ...
        mvae_model.eval()
        smpl_motion_latent = []
        for strt_idx in range(0,seq_len-overlap,seq_len-25):
