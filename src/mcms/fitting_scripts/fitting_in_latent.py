...
with trange(big_seq_start,big_seq_end,fps_scl*(seq_len-overlap)) as seq_t:
    for seq_start in seq_t:
        ...
        if config["optimize_intr"]:
            focal_len = torch.tensor([300 for _ in range(num_cams)]).float().requires_grad_(True)
        else:
        ...
        smpl_motion_latent = smpl_motion_latent.detach().requires_grad_(True)
        smpl_shape = smpl_shape.detach().requires_grad_(True)
        
        ################# Optimizer #################
        if config["optimize_intr"]:
            optim = torch.optim.Adam(cam_orient + cam_position + [focal_len, smpl_motion_latent, smpl_shape],lr=lr)
        else:
            optim = torch.optim.Adam(cam_orient + cam_position + [smpl_motion_latent, smpl_shape],lr=lr)
