...
if dset.lower() == "h36m":
    ...
    ds = savitr_dataset.savitr_dataset(config["data_path"],seq_len=25,used_cams=config["cams_used"])
    ...
    fps_scl = 1
    viz_dwnsample = 1
renderer = [Renderer(img_res=[np.ceil(res[0]/viz_dwnsample),np.ceil(res[1]/viz_dwnsample)]) for res in im_res]
...
def load_batch_wth_pare_init():
    ...
    with trange(big_seq_start,big_seq_end,fps_scl*(seq_len-overlap)) as seq_t:
        for seq_start in seq_t:
            ...
            j2ds = batch["j2d"].float().to(device)
            init_stage_j2d.append(j2ds.detach())
            ...
            for c in range(len(config["cams_used"])):
                if c in config["position_changing_cams"]:
                    cam_position.append(torch.tensor([0,0,5]).float().repeat(1,seq_len,1).to(device))
                else:
                    cam_position.append(torch.tensor([0,0,5]).float().repeat(1,1,1).to(device))
            ...
            cam_orient = [cam_orient[c].detach().unsqueeze(0).repeat([1,seq_len,1,1]) if c in config["orient_changing_cams"] 
                            else cam_orient[c,0:1].detach().unsqueeze(0) for c in range(num_cams)]
...
def optimize(params,device, iters):
    ...
    cam_orient = [p3d_rt.matrix_to_rotation_6d(x).detach().to(device).requires_grad_(True) for x in params["cam_orient"]]
    cam_position = [x.detach().to(device).requires_grad_(True) for x in params["cam_position"]]
    smpl_trans_ff = params["smpl_trans"][:1].detach().to(device)
    smpl_orient_ff = p3d_rt.matrix_to_rotation_6d(params["smpl_orient"][:1]).detach().to(device)
    smpl_trans_rf = params["smpl_trans"][1:].detach().to(device).requires_grad_(True)
    smpl_orient_rf = p3d_rt.matrix_to_rotation_6d(params["smpl_orient"][1:]).detach().to(device).requires_grad_(True)
    smpl_shape = params["smpl_shape"].detach().to(device).requires_grad_(True)
    smpl_art_motion_vp_latent = params["smpl_art_motion_vp_latent"].detach().to(device).requires_grad_(True)
    j2ds = params["j2ds"].detach().to(device)
    ...
    with trange(iters[2]) as t:
        for i in t:
            ...
            mvae_model.eval()
            smpl_motion_latent = []
            for strt_idx in range(0,curr_seq_len-seq_len+1,2):
                ...
                curr_root_pose = to_homogeneous(p3d_rt.rotation_6d_to_matrix(smpl_orient[strt_idx:strt_idx+seq_len]),smpl_trans[strt_idx:strt_idx+25])
                ...
                canonical_smpl_motion = torch.cat([canonical_root_pose[:,:3,3],
                                        ...
                                        smpl_motion[strt_idx:strt_idx+seq_len,6:]],dim=1)
    ...
    return {"cam_orient":[p3d_rt.rotation_6d_to_matrix(x).detach().cpu() for x in cam_orient], "cam_position": [x.detach().cpu() for x in cam_position],
                        "smpl_trans":smpl_trans.detach().cpu(), 
                        "smpl_orient": p3d_rt.rotation_6d_to_matrix(smpl_orient).cpu(), "smpl_shape": smpl_shape.detach().cpu(), 
                        "smpl_art_motion_vp_latent":smpl_art_motion_vp_latent.detach().cpu(), "j2ds":j2ds.detach().cpu(),
...
def stitch(prev_stage_dicts,stitch_num):
    ...
    with torch.no_grad():
        ...
        for n_dict in range(0,len(prev_stage_dicts)-stitch_num+1,stitch_num):
            ...
            cam_orient = [x.detach() for x in prev_dict["cam_orient"]]
            cam_position = [x.detach() for x in prev_dict["cam_position"]]
            smpl_trans = prev_dict["smpl_trans"].detach()
            smpl_shape = prev_dict["smpl_shape"].detach()
            smpl_orient = prev_dict["smpl_orient"].detach()
            smpl_art_motion_vp_latent = prev_dict["smpl_art_motion_vp_latent"].detach()
            last_smpl_orient = prev_dict["smpl_orient"].detach()
            last_smpl_trans = prev_dict["smpl_trans"].detach()
            j2ds = prev_dict["j2ds"].detach()
            ...
            for i in range(1,stitch_num):
                ...
                smpl_trans = torch.cat([smpl_trans[:-overlap],tfmd_smpl[:,:3,3]]).detach()
                ...
                j2ds = torch.cat([j2ds[:,:-overlap],prev_dict["j2ds"].detach()],dim=1)
...
def save_results(stage_dict,stage,seq_start, viz=False):
    ...
    if viz:
        for cam in tqdm(range(num_cams)):
            ...
            for s in range(curr_seq_len):
                ...
                temp_intr = cam_intr[cam].detach().cpu().numpy()
                temp_v = smpl_out_v.view(curr_seq_len,6890,3)[s].detach().cpu().numpy()
                temp_v[:,2] = temp_v[:,2]
                ...
                cv2.imwrite("/is/ps3/nsaini/projects/mcms/mcms_logs/fittings/{}/{:04d}/stage_{:02d}/seq_start_{:05d}/cam_{:02d}/{:05d}.png".format(config["trial_name"],
                                    seq_no,stage,seq_start,cam,seq_start+s),rend_ims[::5,::5,::-1]*255)

    pkl.dump(stage_dict,open("/is/ps3/nsaini/projects/mcms/mcms_logs/fittings/{}/{:04d}/stage_{:02d}/seq_start_{:05d}.pkl".format(config["trial_name"],
                                seq_no,stage,seq_start),"wb"))
    np.savez("/is/ps3/nsaini/projects/mcms/mcms_logs/fittings/{}/{:04d}/stage_{:02d}/seq_start_{:05d}".format(config["trial_name"],
                                seq_no,stage,seq_start),
        verts=smpl_out_v.detach().cpu().numpy(),
...
if __name__ == "__main__":
    ...
    stage = 0
    iterations = [config["n_optim_iters_stage0"],config["n_optim_iters_stage1"],config["n_optim_iters"]]
    ...
    for i in tqdm(range(len(pare_init_state))):
        # res_stage.append(optimize(pare_init_state[i],device))
        print("##### saving results ######")
        save_results(res_stage[i],stage,big_seq_start + i*(seq_len-overlap))
    fitter_res_stage.append(res_stage)
    stitched_res = stitch(fitter_res_stage[-1],2)

    print("\n")
    print("############### stage length {} ##############".format(len(res_stage)))
    print("############### stitched length {} ##############".format(len(stitched_res)))
    print("\n")

    iterations = [config["n_optim_iters_stage0"]//2,config["n_optim_iters_stage1"]//2,config["n_optim_iters"]//2]
    while len(res_stage) != 1:
        if len(stitched_res) < 4:
            print("device is swtiched to CUDA")
            device = "cuda"
            # res_stage = []
            # for i in tqdm(range(len(stitched_res))):
            #     res_stage.append(optimize(stitched_res[i],device))
        ...
        fitter_res_stage.append(res_stage)
        stitched_res = stitch(fitter_res_stage[-1],3)
