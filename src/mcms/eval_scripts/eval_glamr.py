...
# load results
res = pkl.load(open(ospj(glamr_res_dir,"grecon","vid_seed1.pkl"),"rb"))["person_data"][0]
res_global_orient = torch.from_numpy(res["smpl_orient_world"]).float().to(device)[start_offset+config["big_seq_start"]:batch["gt_body_pose"].shape[0]+config["big_seq_start"]]
res_trans = torch.from_numpy(res["root_trans_world"]).float().to(device)[start_offset+config["big_seq_start"]:batch["gt_body_pose"].shape[0]+config["big_seq_start"]]
cam_wrt_res_root = torch.inverse(to_homogeneous(p3d_rt.axis_angle_to_matrix(
                    torch.from_numpy(res["smpl_orient_cam_in_world"][start_offset+config["big_seq_start"]:batch["gt_body_pose"].shape[0]+config["big_seq_start"]])),
                    torch.from_numpy(res["root_trans_cam_in_world"][start_offset+config["big_seq_start"]:batch["gt_body_pose"].shape[0]+config["big_seq_start"]])).float().to(device))


res_body_pose = torch.from_numpy(res["smpl_pose"]).float().to(device)[start_offset+config["big_seq_start"]:batch["gt_body_pose"].shape[0]+config["big_seq_start"]]

res_betas = torch.from_numpy(res["smpl_beta"][:,:10]).float().to(device)[start_offset+config["big_seq_start"]:batch["gt_body_pose"].shape[0]+config["big_seq_start"]]
...
np.savez(ospj(res_dir,"gt_and_glamr_res_strt_off_"+str(start_offset)),verts=torch.stack([gt_smplout.v,res_smplout.v]).detach().cpu().numpy(),
...
yaml.safe_dump(res_dict,open(ospj(res_dir,"gt_and_glamr_res_strt_off_"+str(start_offset)+".yml"),"w"))
